...
# 11.训练部署
def train(gen_model,dis_model,gan_model,dataset,latent_dim,Experiment,n_epochs=5,batch_size=128):
    ...
    plt.figure
    plt.plot(d_loss1_history)
    plt.plot(d_loss2_history)
    plt.plot(g_loss_history)
    plt.legend(['d_loss1','d_loss2','g_loss'])
    plt.xlabel('epochs')
    plt.ylabel('loss')
    plt.savefig('./Experiment%d/Experiment%d_loss.jpg'%(Experiment+1,Experiment+1))
    plt.close()
    plt.figure
    plt.plot(acc_real_history)
    plt.plot(acc_fake_history)
    plt.legend(['acc_real','acc_fake'])
    plt.xlabel('epochs')
    plt.ylabel('accuracy')
    plt.title('Real & Fake Accuray by discriminator')
    plt.savefig('./Experiment%d/Experiment%d_Accuracy.jpg'%(Experiment+1,Experiment+1))
    plt.close()    
...
for Experiment in range(9,10):
    ...
    dis_model = load_model(r'./Experiment5/Experiment5_discriminator_model_600.h5')
    #gen_model = define_generator(latent_dim)
    gen_model = load_model(r'./Experiment5/Experiment5_generator_model_600.h5')
    #gan_model = define_gan(gen_model,dis_model)
    gan_model = load_model(r'./Experiment5/Experiment5_gan_model_600.h5')
