...
# 使用保存的模型继续训练
batch_size = 32
model_name = 'colorized_ae_model2'
save_dir = os.path.join(os.getcwd(),'saved_models')
model = load_model('saved_models\colorized_ae_model')
if not os.path.isdir(save_dir):
    os.mkdir(save_dir)
filepath = os.path.join(save_dir,model_name)
lr_reducer = ReduceLROnPlateau(factor=np.sqrt(0.1),cooldown=0,patience=5,verbose=1,min_lr=0.5e-6)
...
checkpoint = ModelCheckpoint(filepath=filepath,monitor='val_loss',verbose=1,save_best_only=True)
callbacks = [lr_reducer,checkpoint]
model.fit(X_train_gray,X_train,validation_data=(X_test_gray,X_test),epochs=1,batch_size=batch_size,callbacks=callbacks)
